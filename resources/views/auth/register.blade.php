        ...
        <div class="login-right">
            <div class="card">
                ...
                <form method="POST" action="{{ route('register') }}" style="margin-top:18px">
                    ...
                    <!-- BLOK USERNAME YANG BARU DITAMBAHKAN -->
                    <div class="form-group">
                        <label class="small">Username</label>
                        <input id="username" type="text" class="form-input" name="username"
                            value="{{ old('username') }}" required placeholder="Pilih username unik">
                        @if ($errors->has('username'))
                            <div class="small" style="color:#c0392b">{{ $errors->first('username') }}</div>
                        @endif
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input id="email" type="email" class="form-input" name="email" value="{{ old('email') }}"
                            required placeholder="hello@matchaboy.com">
                        @if ($errors->has('email'))
                    ...
                    <div style="display: flex; gap: 12px;">
                        ...
                        <div class="form-group" style="flex: 1;">
                            ...
                            @if ($errors->has('password_confirmation'))
                                <div class="small" style="color:#c0392b">{{ $errors->first('password_confirmation') }}
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div style="display:flex; align-items:center; gap:8px; margin-top:6px;">
                        <label style="display:flex; align-items:center; gap:8px; font-size:13px; color:#6b7a6b">
                            <input type="checkbox" name="terms" required> I agree to the Terms of Service and Privacy
                            Policy
                        </label>
                    </div>

                    <div class="form-actions">
                        <div class="muted small">Already part of the ritual? <a href="{{ route('login') }}"
                                style="color:#365b40; font-weight:600"> Login</a></div>
                        <button type="submit" class="btn-primary">Create Account →</button>
                    </div>
                </form>

                <div
                    style="margin-top:24px; padding-top:16px; border-top:1px solid #e6ece3; display:flex; justify-content:space-between; align-items:center;">
