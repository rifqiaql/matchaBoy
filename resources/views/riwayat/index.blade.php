...
@section('content')
    <div class="p-6">
        ...
        @if (session('success'))
            <div
                class="mb-4 p-4 text-green-700 bg-green-50 border border-green-200 rounded-lg font-medium flex items-center gap-2">
        ...
        @if (session('error'))
            <div
                class="mb-4 p-4 text-red-700 bg-red-50 border border-red-200 rounded-lg font-medium flex items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-5 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                ...
                <form action="{{ route('riwayat.index') }}" method="GET" class="flex items-center gap-3">
                    <select name="month"
                        class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-[#86A789] focus:border-[#86A789] text-sm outline-none">
                        @for ($i = 1; $i <= 12; $i++)
                    ...
                    <button type="submit"
                        class="px-5 py-2 bg-[#86A789] hover:bg-[#739276] text-white text-sm font-semibold rounded-lg transition-colors">
                        Filter Data
                    </button>
                </form>
            </div>
                        ...
                        @forelse($transactions as $index => $t)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-sm text-center">
                                    <form action="{{ route('riwayat.void', $t->id) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="inline-flex items-center gap-2 px-3 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 font-medium rounded-md transition-colors border border-red-200 focus:ring-2 focus:ring-red-200 outline-none">
